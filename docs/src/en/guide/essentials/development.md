...
```json
{
  "scripts": {
    ...
    "build:naive": "pnpm run build --filter=@kris/naive",
...
Take the addition of the test environment `test` to `@kris/web-antd` as an example:
...
"scripts": {
    ...
    "build:analyze": "turbo build:analyze",
    "build:antd": "pnpm run build --filter=@kris/web-antd",
    "build-test:antd": "pnpm run build --filter=@kris/web-antd build:test",
...
"tasks": {
    ...
    "build-test:antd": {
      "dependsOn": ["@kris/web-antd#build:test"],
      "outputs": ["dist/**"]
    },

    "@kris/web-antd#build:test": {
      "dependsOn": ["^build"],
      "outputs": ["dist/**"]
    },
